  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (request.method === "POST" && url.pathname === "/api/telemetry") {
        ...
        ctx.waitUntil(
          env.DB.prepare(
            ...
            userAgent,
            timestamp
          ).run().catch((err) => console.error("D1 event insert error:", err))
        );
      ...
      if (request.method === "POST" && url.pathname === "/api/feedback") {
        ...
        ctx.waitUntil(
          env.DB.prepare(
            ...
            body.path || null,
            timestamp
          ).run().catch((err) => console.error("D1 feedback insert error:", err))
        );
        return jsonResponse({ success: true, id }, 202);
      }
      if (request.method === "GET" && url.pathname === "/api/telemetry/pull") {
        ...
        const limit = Math.min(Number(url.searchParams.get("limit") || 1e3), 5e3);
